...
from aica_django.connectors.Nmap import network_scan  # noqa: F401
...
def handle_suricata_alert(alert_dict: dict) -> bool:
    ...
    recommended_actions: List[dict] = query_action(alert_dict)

    # Check with behavior engine
    approved_actions = list()
    for action in recommended_actions:
        action["score"] = query_rules(alert_dict, action)
        if action["score"] >= 0:
    ...
    if len(approved_actions) > 0:
        ...
        for action in actions_to_take:
            if action["action"] == "honeypot":
                ...
                logger.info(f"Initiating return scan to {alert_dict['src_ip']}")
                current_app.send_task("network-scan", nmap_target=alert_dict["src_ip"])
                return True
            elif action["action"] == "scan_target":
                logger.info(f"Initiating scan of target at {alert_dict['dest_ip']}")
                current_app.send_task("network-scan", nmap_target=alert_dict["dest_ip"])
                return True
